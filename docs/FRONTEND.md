...
Reviewing a candidate opens `CardZoomModal`, a linked pan/zoom comparison: the user's photo and the candidate scan zoom and pan together as one shared transform (scroll or click to zoom toward the cursor up to 6x, drag to pan, arrow keys to step through the other candidates for that photo without losing the zoom position, Escape backs out of the zoom before closing the modal). Its Accept button opens the existing add-to-collection modal rather than resolving directly — there is still no auto-accept. Candidate images progressively load: `useCandidateFullImage` shows the already-cached thumbnail blurred as a stand-in while the full-resolution image (from the backend's candidate-image cache endpoint) decodes, and `usePrefetchMatchImages`/hover-triggered prefetching warm the browser cache ahead of time. `JobDetail` in `ScanQueue.jsx` (not the individual item panel) owns which photo/candidate is under review, because accepting a match from the zoom view auto-advances to the next unresolved photo with candidates in the same job (`openNextReview`), letting a reviewer clear a whole batch without returning to the list between cards. Once an item is resolved it collapses to a compact one-line row in `ScanItemPanel`, re-expandable on click, so a long batch stays scannable. Each photo also has a manual 90° rotate button — recognition straightens photos automatically only when the matched card has a reference scan to compare against, so there is no reliable automatic fallback for the rest.
